...
#             self.color_cut_flag = 0
        ...
        if not self.opts.no_coarse_mapper:
            x_coarse = self.course_mapping(x_coarse, txt_embed[:, :4, :], cut_flag=self.hairstyle_cut_flag)
        else:
            x_coarse = torch.zeros_like(x_coarse)
        if not self.opts.no_medium_mapper:
            x_medium = self.medium_mapping(x_medium, txt_embed[:, 4:8, :], cut_flag=self.hairstyle_cut_flag)
        else:
            x_medium = torch.zeros_like(x_medium)
        if not self.opts.no_fine_mapper:
            x_fine = self.fine_mapping(x_fine, txt_embed[:, 8:, :], cut_flag=self.color_cut_flag)
        else:
